...
pub async fn websocket_proxy(
    ...
    //TODO match if request is websocket upgrade or http request
    // if ws.is_some() {
    //     let ws = ws.unwrap();
    ...
    //         let http = http.unwrap();
        ...
        let d = ReusedService::call(&mut proxy, http).await.unwrap().unwrap();
        d.into_response()
    // } else {
    //     panic!("neither http nor websocket request");
    // }
}
