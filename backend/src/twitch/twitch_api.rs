...
impl TwitchService {
    pub async fn start_websocket(state: Arc<FullState>, commands_sender: Sender<Box<ChatMessage>>) {
        let process_messages = async move |state: State<Arc<FullState>>, message: Event<ChannelChatMessage>| {
            ...
            let c = ChatMessage {
                ...
                channel_id: message.source_broadcaster_user_id
                    ...
                    .unwrap_or(state.l2.twitch_service.config.channel_name.clone().into_boxed_str()),
